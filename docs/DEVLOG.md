...
- **Offline assets**: `scripts/setup_speech_assets.mjs` (`npm run setup:speech`) copies the VAD
  worklet + Silero ONNX → `app/public/vad/` and onnxruntime-web wasm → `app/public/ort/`; the
  worker points ORT at `/ort/` in single-threaded mode (no COOP/COEP needed). The Whisper model
  downloads + caches on first use (drop a copy under `public/models/` for fully-offline first run).
